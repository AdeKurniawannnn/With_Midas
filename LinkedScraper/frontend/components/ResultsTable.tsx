...
export function ResultsTable({ profiles, metadata }: ResultsTableProps) {
  ...
  // Detect apakah ini company atau profile data
  // Check first item - jika ada industry/followers/company_size, berarti company
  const isCompanyData = profiles.length > 0 &&
    (profiles[0].industry !== null ||
     profiles[0].followers !== null ||
     profiles[0].company_size !== null);
  ...
  const handleExportCSV = () => {
    ...
    if (isCompanyData) {
      // Company CSV
      const headers = ["No", "Selected", "Nama Perusahaan", "Industri", "Followers", "Ukuran Perusahaan", "Tahun Berdiri", "Lokasi", "Company URL"];
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                {isCompanyData ? (
                  <>
                    <TableHead className="w-48">Industri</TableHead>
